...
usage() {
  cat <<'EOF'
用法:
  scripts/pr-review-poller.sh [--dry-run] [--state-file <path>] [--no-post-review]

说明:
  检查当前仓库开放 PR 的最新 head SHA。
  只有当某个 PR 自上次审查后出现新提交时，才触发一次 review。
  如果没有开放 PR，或所有 PR 都没有新提交，则直接结束。
EOF
}
...
load_open_prs() {
  gh pr list --state open --json number,title,headRefOid,author,isDraft,url
}
