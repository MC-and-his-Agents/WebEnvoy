# 读写下载机制与 Content Script 上下文

> 所属文档：[系统设计（战术层）](../system-design.md) › 第四至六章
> 覆盖章节：§四 读操作机制 · §五 写操作机制 · §五补 下载机制 · §六 Content Script 执行上下文
...
下载能力属于独立的核心能力面，但它不应另起一套平行模型。当前设计约束如下：

- **页面内优先**：优先在浏览器页面内完成文件 URL、Blob、媒体资源或导出结果的获取
- **CLI 负责落盘**：Content Script / Extension 负责把可下载对象描述回传给 CLI，由 CLI 统一决定文件命名、保存路径与运行记录
- **统一结果模型**：下载结果应和读 / 写一样具备统一的能力返回结构，至少包含来源 URL、媒体类型、文件名建议、落盘结果和失败原因

**典型路径**：

1. 页面内直接拿到可访问的资源 URL
2. 页面内拿到 `blob:` URL 或 `Blob` 对象并转成可传输内容
3. 页面触发导出任务后，拦截真实下载链接或生成结果

**边界约束**：

- 不把浏览器扩展自身的下载管理 API 作为唯一实现前提
- 不允许为了下载而绕开“浏览器内执行是唯一 HTTP 出口”原则
- 下载落盘后只在 SQLite 中记录路径和元数据，不把大文件写入数据库
