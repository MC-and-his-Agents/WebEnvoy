...
| official Chrome runtime migration implementation-prep | `Phase 1.x / #426 -> FR-0015 / #435` |

### 11.4 当前单一主树映射

为避免完整体系继续散落在架构描述中，当前 GitHub 结构统一收敛为 `Phase -> FR -> Work Item`：

| Phase | Canonical Issue | 当前承接 FR |
| --- | --- | --- |
| `Phase 1.x` | `#426` | `FR-0009 / #215`、`FR-0010 / #230`、`FR-0011 / #231`、`FR-0015 / #435` |
| `Phase 2` | `#427` | `FR-0012 / #265`、`FR-0013 / #267`、`FR-0014 / #266`、`FR-0020 / #239` |
| `Phase 4` | `#423` | `FR-0022 / #238` |

对应 Work Item 直接挂回 owning FR。历史 `#232/#233/#234/#368` 只保留 closed migration note，不再承担当前父级结构。

### 11.5 哪些能力进入近期 backlog，哪些只冻结在蓝图

**进入近期 backlog：**

- Layer 1 JS 指纹补全与 profile 一致性（`Phase 2 / #427 -> FR-0012 / #265`）
- Layer 2 事件级拟人模拟增强（`Phase 2 / #427 -> FR-0013 / #267`）
- Layer 3 完整 session 行为节律引擎（`Phase 2 / #427 -> FR-0014 / #266`）
- 反风控验证与基线评估（`Phase 2 / #427 -> FR-0020 / #239`）

**进入后层扩展，但现在就冻结到蓝图：**

- Layer 4 平台行为模型与长期基线（`Phase 4 / #423 -> FR-0022 / #238`）
...
- 后续若新增反风控 backlog，应直接挂到 owning `Phase -> FR -> Work Item` 主树，不再新增 umbrella 或平行父级 issue。
- 历史 `#232/#233/#234/#368` 只保留 closed migration note，不再承担当前父级结构。
