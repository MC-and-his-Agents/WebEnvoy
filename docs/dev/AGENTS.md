...
- `Issue` 页面先承载关系、状态、关闭语义与 `Spec Path` 等核心元数据；`Project` 只负责横切视图
- repo 级治理、脚本、review 门禁事项也必须找到 owning `FR`；若横跨多个 FR，应先在对应 `Phase` 下建立治理型 `FR`
- 旧结构标签只保留历史兼容，不再表达当前层级
...
5. Spec 合入后，由 CI 仅按 `.github/spec-issue-sync-map.yml` 单向同步到 canonical FR issue；未映射的 spec 直接阻断。
