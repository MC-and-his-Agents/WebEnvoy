...
`#354` 已收口 FR-0001 的 CLI 外层契约（命令调用、`stdout`/`stderr` 边界、统一错误壳与 `run_id`）。历史 issue `#159` 只保留为“能力壳方向最早来源”的引用，不再承接当前 formal closeout；当前 formal 承接 issue 为 `#360`。同样，issue `#360` 中保留的 `Refs #154` 只代表历史链路引用，FR-0007 当前对接的最小诊断正式边界以后继 issue `#357` / FR-0004 为准，证据层映射边界以后继 issue `#359` / FR-0006 为准。FR-0007 的职责是在不破坏 FR-0001 的前提下，补齐“能力层最小封装骨架”，让首个 L3 样本从一开始就具备可复用壳，而不是孤例命令。
...
- `#357` 的观察与诊断增强必须复用 `run_id` 串联能力执行上下文，不重建另一套运行标识，也不要求 FR-0004 复用 FR-0007 的能力专属字段结构。
- FR-0002 已冻结的通信契约与既有实现链路、`#356` 的身份承载实现可并行承接本 FR，但不得改写能力壳字段语义或引入第二套能力入口。
- `#359` 如需为能力结果 / 错误建立落库映射，只能复用本 FR 已冻结的最小字段，不得倒逼响应层引入持久化专属字段。
...
- 硬依赖：
  ...
  - `#145`（首个平台核心读闭环）
  - FR-0004 已冻结的最小观察、错误分类与结构化诊断边界（formal 承接 issue 为 `#357`）
- 并行协同：
  - `#356`
  - FR-0006 formal 收口（承接 issue `#359`）
- 前置文档：
  - `vision.md`
  - `docs/dev/roadmap.md`
