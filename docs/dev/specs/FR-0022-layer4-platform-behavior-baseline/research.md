...
8. `FR-0020` registry 的 shared upstream scope 不包含 `platform`、`target_domain` 或 `goal_kind`；这些键继续属于 `FR-0022` 的 downstream writable isolation。同一条上游 `active_baseline_ref` 可以被多个 `(platform, target_domain, goal_kind)` Layer 4 状态对象并行引用为 shared lineage input，但不能因此折叠这些 scope 的独立状态历史。
