...
- Decision question：在出现平台风险预警后，XHS 读写路径应如何建立统一门禁，才能在不扩张风险的前提下为 `#208/#209` 的后续 formal 套件、恢复讨论与历史同步提供治理基线。
...
- `#208` 已关闭，其后续真实 live 恢复 / replay / 扩展边界由下游 formal 套件继续承接；FR-0009 在本轮只提供治理基线与历史同步口径，不把自己重新定义成当前实施门禁。
...
| 写路径交互 | 真实点击优先、必要时合成事件回退、上传候选路径分级 | `#208` 已闭环其历史 issue，但其后续正式验证边界已在下游 formal 套件中提出 | 写路径仍停在 Spike/候选层（`FR-0008`）及后续 replay / 恢复语义冻结阶段 | 在证据不足时继续 live 写实验，风险放大且难归因 |
...
- Rationale：
  ...
  - `#208/#209` 的后续 formal 套件、恢复讨论与历史同步都需要引用本 FR 的治理基线。
