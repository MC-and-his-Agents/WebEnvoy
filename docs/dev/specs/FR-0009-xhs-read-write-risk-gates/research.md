...
- `#209` 已闭环并关闭：读路径具备可用性，但不代表其后续 live 扩展风险可忽略。
- `#208` 已作为 Phase 1 最小页面交互验证事项关闭：这不自动等于后续真实 live 恢复 / replay / 扩展可直接放行。
- `#201` 已关闭：其 Phase 1 exit review 语义仍可作为历史消费者引用，但不再作为当前 sprint 真相源。
- 风险事实：已出现平台侧账号风险预警样本。
...
|---|---|---|---|---|
| 读路径模式 | `API primary + page fallback + 被动拦截辅路径` | `xhs.search` 的 API 主路径已实现并合入，`#209` 已关闭 | 被动拦截辅路径尚未成为 XHS 主执行收敛策略；后续 live 扩展仍需消费统一门禁 | 当前 live 行为更集中在“工具驱动 API 请求”，行为侧风险更聚焦 |
| 写路径交互 | 真实点击优先、必要时合成事件回退、上传候选路径分级 | `#208` 作为 Phase 1 历史事项已关闭，但不代表 live 恢复条件已天然满足 | 写路径仍停在 Spike/候选层（`FR-0008`）及后续 replay / 恢复语义冻结阶段 | 在证据不足时继续 live 写实验，风险放大且难归因 |
