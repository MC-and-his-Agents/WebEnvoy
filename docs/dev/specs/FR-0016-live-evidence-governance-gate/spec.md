...
- 若同一 PR 同时改动 `docs/dev/specs/FR-0016-live-evidence-governance-gate/**`，且又命中“五处治理落库目标文件 + `#310` issue 引用”这一 FR-0016 落库条件，必须作为 `mixed_spec_and_governance_scope` 直接阻断；formal spec review PR 与治理落库 PR 不得在同一条高风险链路重新混线。
