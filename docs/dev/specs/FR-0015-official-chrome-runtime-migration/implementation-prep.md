...
GitHub backlog 由 `#361` 承接本文件对应的 implementation-prep 第一刀停点。本文档继续负责冻结实现输入；下游实现 issue / PR 应显式挂接 `#361`，而不是继续把第一刀 backlog 留在本地 `TODO.md`。
...
- `#361` 承接的 scope 只允许覆盖 identity preflight、`runtime_bootstrap_envelope` contract、`runtime.status` readiness read model，以及 bootstrap 失败后的 stop/retry/recover 边界。
- `#361` 的 stop-ship 与验证入口继续分别以本文件第 4/5 节、`plan.md` 的健康矩阵与恢复路径、`risks.md` 的 stop-ship 条件为准，不在 issue 或 `TODO.md` 之外另起口径。
- 历史实现链路继续以 `#281` 及其已合并 PR 为准；后续若仍需补第一刀实现或验证 follow-up，应在 scope 不外扩的前提下显式 `Refs #361`。
