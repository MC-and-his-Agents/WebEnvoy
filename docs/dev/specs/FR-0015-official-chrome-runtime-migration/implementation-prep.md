...
GitHub backlog 由 `#361` 承接本文件对应的 implementation-prep 第一刀停点，但 `#281` 继续是 FR-0015 runtime migration 的主线约束 issue。本文档继续负责冻结实现输入；下游实现 issue / PR 可以显式挂接 `#361`，但不得把它写成对 `#281` 的替代，也不应继续让本地 `TODO.md` 承担 backlog 真相源。
...
- `#361` 的 scope、stop-ship、验证入口、关闭条件与下游挂接关系，必须与本文件、`spec.md`、`plan.md`、`contracts/`、`risks.md` 以及 issue 正文保持一致，不在这里另起缩写版本。
- 历史实现链路继续以 `#281` 及其已合并 PR 为准；后续若仍需补第一刀实现或验证 follow-up，应在不外扩 scope 的前提下同时显式 `Refs #281` 与 `Refs #361`。
