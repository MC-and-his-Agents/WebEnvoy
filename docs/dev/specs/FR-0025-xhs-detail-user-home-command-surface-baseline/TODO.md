...
- [ ] reviewer 确认 successor detail implementation path 必须先消费 `#504 + #505` merged baselines，再继续等待 `#508 + #510` 两条 open formal gate；其中 `#508` 只承接 shared request-context minimal invariants，`#510` 只作为 required detail-path gate 引用，不在本 FR 内重述其 owning suite scope
