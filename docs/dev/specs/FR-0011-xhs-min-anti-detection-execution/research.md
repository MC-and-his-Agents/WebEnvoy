...
- 集成约束：`#223` 要求 `#208/#209` 接入统一风险状态机与阻断策略。
...
| U5 | `#208/#209` 可共享同一状态机并保持差异化阻断 | `#223` issue 约束 + FR-0011 contract 草案 | 契约可判定性审查 | M2 | 86% | 通过 issue_action_matrix 固化 |
...
| 统一集成 | `#208/#209` 需共同受控 | `#223` 约束下的差异化阻断矩阵与审计字段 |
...
- Rationale：
  ...
  - 先在规约层收口 `#223` 所需的统一状态机对象，降低实现阶段口径分叉风险。
