...
- [ ] `#209` 的 live read admission evidence 与 post-gate persisted trail 必须分别落在已冻结的 `FR-0010.gate_input.admission_context` 下的 request-side `approval_admission_evidence` / `audit_admission_evidence` 与 `FR-0010.approval_record` / `FR-0010.audit_record` 上；后续实现与 review 不得再把两者混写成同一时序对象。
