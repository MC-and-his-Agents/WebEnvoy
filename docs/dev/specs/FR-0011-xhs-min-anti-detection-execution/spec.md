...
- 必须明确 `issue_209` 的 live read admission evidence 与 gate 后 persisted audit trail 是两个正式阶段：前者使用独立的 pre-gate audit payload，后者继续使用 `FR-0010.audit_record` 留痕；两者不得混写为同一个时序对象。
