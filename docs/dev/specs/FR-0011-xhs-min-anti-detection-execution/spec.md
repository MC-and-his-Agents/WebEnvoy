...
`FR-0009`（`#213`）已经冻结了读写路径风险门禁规约，并明确在门禁落实前不得恢复高风险 live 读写实验。当前缺口是“最小反风控执行能力”仍未形成可实现的正式契约，导致后续 `#208`（最小页面交互正式验证）和 `#209`（读路径后续 live 扩展）缺少统一可执行前置。`#223` 进一步要求把 `#208/#209` 接入同一风险状态机与阻断策略，避免两条线口径漂移。
...
6. 明确承载 `#223`：把 `#208/#209` 纳入同一状态机输入、阻断矩阵与恢复条件。
