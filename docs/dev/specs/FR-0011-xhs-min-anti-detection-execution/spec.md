...
- 必须明确 `issue_209` 的 live read admission evidence 与 gate 后 persisted trail 是两个正式阶段：前者作为 `FR-0010.gate_input.admission_context` 的 request-side 输入，承载独立的 pre-gate `approval_admission_evidence` / `audit_admission_evidence`；后者继续使用 `FR-0010.approval_record` / `FR-0010.audit_record` 留痕；两者不得混写为同一个时序对象。
