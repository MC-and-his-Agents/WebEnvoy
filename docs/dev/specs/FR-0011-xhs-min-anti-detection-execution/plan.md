...
6. `issue_209` 的 pre-gate admission evidence 必须被明确分类为 `FR-0010.gate_input.admission_context` 下的 request-side input，不得与 `FR-0010` 的 gate 后 `approval_record` / `audit_record` 混写为同一对象族。
...
- 审查验证：
  ...
  - GWT 场景可被 reviewer 直接判定
    ...
    - `issue_209` 的 admission evidence 是否已被整套 formal 文档一致地定义为 `FR-0010.gate_input.admission_context` 下的 request-side input，而非 gate 后输出对象
...
7. `live_read_limited` 的公开模式语义、条件载体 `limited_read_rollout_ready_true`、`FR-0010.gate_input.admission_context` 下的 request-side admission evidence 要求与 `blocked` 时的 `effective_execution_mode` 语义已冻结，可作为后续实现事项的正式准入前置。
