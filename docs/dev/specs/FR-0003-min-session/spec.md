...
- FR-0003 冻结 Named Profile、最小身份 / 会话模型和 `__webenvoy_meta.json` 基线字段；FR-0015 只是后续 formal 加项案例之一，可在 official Chrome persistent extension 迁移语境下新增 `persistentExtensionBinding`。
...
- FR-0003 基线下，`__webenvoy_meta.json` 只允许出现以下顶层字段白名单：
  ...
  - `lastStoppedAt`
  - `lastDisconnectedAt`
- 上述白名单之外的顶层字段，只有在后续 FR 经 formal spec review 明确冻结后，才允许作为加性可选字段出现；例如 FR-0015 的 `persistentExtensionBinding`，以及 FR-0012 formal 套件另行定义的指纹一致性字段，都不属于 FR-0003 原生白名单。
