...
- `runtime.login` 负责打开或保持可见浏览器，让用户完成一次手动登录并将结果持久化回 Profile。
...
- `runtime.login` 可以把 `ready` 转入 `logging_in`，登录确认后回到 `ready`。
...
2. `runtime.login` 能把手动登录结果保存回同一 Profile。
