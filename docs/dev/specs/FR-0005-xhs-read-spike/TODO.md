...
- [x] 仓库内已固化样本中的“`detail/user_home` 无公开 CLI 命令入口”结论，已在 `research.md` 标记为 dated historical fact；当前 implementation base `a4a5cfce92bf9a0e9fae016543f007e738922083` 的公开命令面已包含 `xhs.detail` / `xhs.user_home`
- [x] 2026-04-14 的当前 docs-only PR latest-head exploratory rerun 已确认：`search/detail/user_home` 三场景都具备合法 fresh rerun 路径；当前直接阻断分别为 `search=live_read_blocked_by_risk_state`、`detail/user_home=repo_latest_head_execution_bundle_failure(buildIssue209PostGateArtifacts is not defined)`
- [x] 上述 latest-head exploratory rerun 同时确认：三场景都仍未达到 `route_role=primary + path_kind=api + evidence_status=success + reproduced_multi_round`，因此 formal 结论继续 `No-Go/paused`
...
- [x] 在当前 docs-only PR latest head 上，已按独立场景 rerun 确认 `xhs.detail` / `xhs.user_home` 的公开 command surface 已形成合法 fresh rerun 路径；旧的“无公开 CLI 入口”停点已降级为历史事实
- [x] 在当前 docs-only PR latest head 上，已按独立场景 rerun 确认直接阻断改写为：`search` 被 runtime/risk gate 阻断，`detail/user_home` 被 latest-head execution bundle failure 阻断
- [x] 在当前 docs-only PR latest head 上，已再次判定 `search/detail/user_home` 仍未达到 `route_role=primary + path_kind=api + evidence_status=success + reproduced_multi_round`
- [ ] 若 docs 或 PR body 继续前移 head，必须在新的 `pr_latest_head_sha` 上重做 `search/detail/user_home` 独立 fresh rerun，并把 current gate evidence 只写入 PR `live_evidence_record`
...
- [ ] 在新的 latest head 上复核 `detail/user_home` 的直接阻断是否仍为 `buildIssue209PostGateArtifacts is not defined`，或已转为 runtime/risk gate、target binding、API failure
