...
describe("xhs.editor_input", () => {
  const baseContext: RuntimeContext = {
    run_id: "run-editor-input-test-001",
  ...
  const withLoopbackRuntimeEnv = async <T>(fn: () => Promise<T>): Promise<T> => {
  ...
  it("returns invalid args when text is missing", async () => {
    await withLoopbackRuntimeEnv(async () => {
      await expect(
        executeCommand(
          {
            ...baseContext,
        ...
        details: {
          reason: "EDITOR_TEXT_MISSING"
        }
      });
    });
  });

  it("returns invalid args when action_id is not editor_input", async () => {
    await withLoopbackRuntimeEnv(async () => {
      await expect(
        executeCommand(
          {
