...
process.stdin.on("data", (chunk) => {
  ...
  itWithSqlite("generates XHS closeout validation source samples through production CLI", async () => {
    const cwd = await createRuntimeCwd();
    const profile = "xhs_validation_source_cli_profile";
    const sourceRunId = "run-xhs-closeout-validation-source-cli-001";
    const validationRunId = "run-xhs-closeout-validation-from-source-cli-001";
    ...
    const expectedSourceSampleRefs = [
      `validation-sample/source/xhs-closeout-min-v1/${profile}/live_read_high_risk/${sourceRunId}/FR-0012/layer1_consistency`,
      `validation-sample/source/xhs-closeout-min-v1/${profile}/live_read_high_risk/${sourceRunId}/FR-0013/layer2_interaction`,
      `validation-sample/source/xhs-closeout-min-v1/${profile}/live_read_high_risk/${sourceRunId}/FR-0014/layer3_session_rhythm`
    ];
    expect(sourceBody).toMatchObject({
      ...
      summary: {
        validation_source_generation: {
          ...
          requested_execution_mode: "live_read_high_risk",
          run_id: sourceRunId,
          action_ref: sourceRunId,
          source_audit_event_id: `gate_evt_${sourceRunId}_xhs_closeout_validation_source`,
          source_session_id: "nm-session-001",
          source_sample_refs: expectedSourceSampleRefs,
          active_fetch_performed: false,
          closeout_bundle_entered: false
        }
      }
    ...
    try {
      const sourceRows = db.prepare(
        `SELECT sample_ref, target_fr_ref, validation_scope, run_id, structured_payload, artifact_refs
        FROM anti_detection_structured_sample
        WHERE sample_ref IN (?, ?, ?)
        ORDER BY target_fr_ref`
      ).all(...expectedSourceSampleRefs) as Array<Record<string, string>>;
      expect(sourceRows).toHaveLength(3);
      for (const row of sourceRows) {
        expect(row.run_id).toBe(sourceRunId);
        ...
        const sourceGateAudit = payload.source_gate_audit as Record<string, unknown>;
        expect(sourceGateAudit).toMatchObject({
          event_id: `gate_evt_${sourceRunId}_xhs_closeout_validation_source`,
          decision_id: `gate_decision_${sourceRunId}_xhs_closeout_validation_source`,
          session_id: "nm-session-001",
          action_ref: sourceRunId,
          target_domain: "www.xiaohongshu.com",
          target_tab_id: 32,
        ...
        expect(sourceGateAudit.gate_reasons).toContain("XHS_CLOSEOUT_VALIDATION_SOURCE_APPROVED");
        const signal = payload.signal as Record<string, unknown>;
        if (row.target_fr_ref === "FR-0012") {
        ...
        if (row.target_fr_ref === "FR-0014") {
          expect(signal).toMatchObject({
            session_rhythm_window_id: `rhythm_win_${profile}_validation_source`,
            session_rhythm_decision_id: `rhythm_decision_${profile}_validation_source`,
            active_fetch_performed: false,
            closeout_bundle_entered: false
          });
        }
    ...
    const validationBody = parseSingleJsonLine(validationResult.stdout);
  ...
  itWithSqlite("fails closed when XHS closeout validation source main-world evidence is invalid", async () => {
    const cwd = await createRuntimeCwd();

    for (const [mode, reason] of [
      [
        "xhs-closeout-validation-source-tab-mismatch",
        "XHS_CLOSEOUT_VALIDATION_SOURCE_TARGET_TAB_MISMATCH"
      ],
      ["xhs-closeout-validation-source-page-mismatch", "XHS_CLOSEOUT_VALIDATION_SOURCE_PAGE_MISMATCH"],
      [
        "xhs-closeout-validation-source-no-response",
        "XHS_CLOSEOUT_VALIDATION_SOURCE_MAIN_WORLD_EVIDENCE_MISSING"
      ],
      [
        "xhs-closeout-validation-source-source-mismatch",
        "XHS_CLOSEOUT_VALIDATION_SOURCE_FINGERPRINT_NOT_READY"
      ],
      [
