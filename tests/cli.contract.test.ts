...
`,
  ...
  it("recovers official Chrome runtime.prepare through the shared bootstrap recovery contract", async () => {
    const runtimeCwd = await createRuntimeCwd();
    const manifestPath = await createNativeHostManifest({
      allowedOrigins: ["chrome-extension://aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa/"]
    });
    const profile = "official_runtime_prepare_profile";
    await seedInstalledPersistentExtension({
      cwd: runtimeCwd,
      profile
    });
    ...
    const start = runCli(
      [
        ...
        profile,
        "--run-id",
        "run-contract-runtime-prepare-start-001",
        "--params",
        JSON.stringify({
          persistent_extension_identity: {
            extension_id: "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",
    ...
    const nativeHostPath = path.join(runtimeCwd, "native-host-runtime-prepare.cjs");
    const tracePath = path.join(runtimeCwd, "native-host-runtime-prepare-trace.json");
    await writeFile(
      nativeHostPath,
      `#!/usr/bin/env node
...
const writeTrace = () => {
  ...
  const existing = require("node:fs").existsSync(tracePath)
    ? JSON.parse(require("node:fs").readFileSync(tracePath, "utf8"))
    : { forwards: [] };
  const mergedForwards = Array.isArray(existing.forwards)
    ? [...existing.forwards, ...forwards]
    : [...forwards];
...
const onRequest = (request) => {
  ...
  if (command === "runtime.readiness") {
    ...
    emit({
      ...
      payload: {
        transport_state: "ready",
        bootstrap_state: bootstrapAttempted && readinessAttempts > 1 ? "ready" : "pending"
      },
      error: null
    });
...
`,
    ...
    expect(parseSingleJsonLine(prepare.stdout)).toMatchObject({
    ...
    expect(trace.forwards).toEqual(
      expect.arrayContaining([
        {
          command: "runtime.bootstrap",
          run_id: "run-contract-runtime-prepare-start-001",
          profile
        },
        {
