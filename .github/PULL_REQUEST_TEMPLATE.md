...
## Live Evidence（仅对声称完成真实 runtime / 真实页面交互 / 真实 live 闭环，或把 live evidence 作为关闭 issue、判定“已完成”或请求 merge 放行依据的 PR 必填；不适用请写 `N/A`）
...
- [ ] 若本 PR 落入真实 live evidence 专项门禁，已补齐 latest head 的有效 live evidence，且未把 stub/fake host、`runtime.ping` 或 `runtime.bootstrap` 误写为真实闭环证据
